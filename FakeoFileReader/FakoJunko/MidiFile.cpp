...
MidiFile::MidiFile(uint8_t * input) {
   ticksPerQuarterNote = 120;            // TQP time base of file
   trackCount = 1;                       // # of tracks in file
   theTrackState = TRACK_STATE_SPLIT;    // joined or split
   theTimeState = TIME_STATE_DELTA;      // absolute or delta
...
int MidiFile::read(FakeFile input) {
   rwstatus = 1;
   if (input.peek() != 'M') {
      //TO BAD!  no failsafe
...
// int MidiFile::write(uint8_t * out) {
...
// int MidiFile::writeHex(uint8_t * out, int width) {
...
// int MidiFile::writeBinasc(uint8_t * output) {
   // stringstream binarydata;
...
int MidiFile::extractMidiData(uint8_t * input, vector<uchar>& array,
   uchar& runningCommand) {
...
ulong MidiFile::readVLValue(uint8_t * input) {
   uchar b[5] = {0};
...
ulong MidiFile::readLittleEndian4Bytes(uint8_t * input) {
   uchar buffer[4] = {0};
...
ushort MidiFile::readLittleEndian2Bytes(uint8_t * input) {
   uchar buffer[2] = {0};
...
uchar MidiFile::readByte(uint8_t * input) {
   uchar buffer[1] = {0};
