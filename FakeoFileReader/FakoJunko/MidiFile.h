...
class MidiFile {
   public:
      ...
      // static functions:
      static uchar    readByte                (uint8_t * input);
      static ushort   readLittleEndian2Bytes  (uint8_t * input);
      static ulong    readLittleEndian4Bytes  (uint8_t * input);
   ...
   private:
      int        extractMidiData  (uint8_t * inputfile, vector<uchar>& array, 
                                       uchar& runningCommand);
      ulong      readVLValue      (uint8_t * inputfile);
