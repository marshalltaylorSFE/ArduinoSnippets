...
SET INPUTPAT=D:\github\ArduinoSnippits\MIDILoader\
SET WORKINGPATH=D:\github\ArduinoSnippits\MIDILoader\output\
REM %AVRPATH%avr-g++
...
"%ARDPAT%hardware\tools\avr\bin\avr-gcc" -w -Os -Wl,--gc-sections -mmcu=atmega328p -o "%WORKINGPATH%MIDILoader.cpp.elf" "%WORKINGPATH%midi1.o" "%WORKINGPATH%MIDILoader.cpp.o" "%WORKINGPATH%core.a" -L"%WORKINGPATH%" -lm 
if %errorlevel% neq 0 goto exitpoint

"%ARDPAT%hardware\tools\avr\bin\avr-objcopy" -O ihex -j .eeprom --set-section-flags=.eeprom=alloc,load --no-change-warnings --change-section-lma .eeprom=0 "%WORKINGPATH%MIDILoader.cpp.elf" "%WORKINGPATH%MIDILoader.cpp.eep"
if %errorlevel% neq 0 goto exitpoint

"%ARDPAT%hardware\tools\avr\bin\avr-objcopy" -O ihex -R .eeprom "%WORKINGPATH%MIDILoader.cpp.elf" "%WORKINGPATH%MIDILoader.cpp.hex" 
if %errorlevel% neq 0 goto exitpoint
